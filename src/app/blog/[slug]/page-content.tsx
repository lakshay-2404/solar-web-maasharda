...
import { getAllBlogPosts, getBlogPostWithContent } from "@/lib/blog";
...
export async function buildBlogPostMetadata(
  ...
  return {
    ...
    openGraph: {
      ...
      locale: language === "en" ? "en_IN" : "hi_IN",
    },
  };
}
...
export async function BlogPostPageContent({
  ...
  const copy = {
    hi: {
      moreReads: "और पढ़ें",
      mobileCtaText: "निःशुल्क सौर योजना चाहिए?",
      mobileCtaButton: "मुफ्त योजना लें",
    },
    en: {
      moreReads: "More reads",
      mobileCtaText: "Need a free solar plan?",
      mobileCtaButton: "Get free plan",
    },
  }[language];
  ...
  return (
    <>
        ...
        <div className="relative h-56 w-full md:h-96">
          ...
          <div className="absolute bottom-6 left-6 right-6">
            ...
            <p className="mt-2 text-sm text-white/70">
              {formatReadTime(post.meta.readTime, language)} ·{" "}
              {formatPublishedDate(post.meta.publishedAt, language)}
            </p>
          </div>
        </div>
        ...
        {related.length > 0 ? (
          <section className="bg-[#f4f4f1]">
            <div className="section-padding mx-auto max-w-6xl">
              <ScrollReveal>
                <h2 className="mb-6 text-xl font-medium text-green-900">{copy.moreReads}</h2>
              </ScrollReveal>
              <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
                {related.map((item) => (
                  <ScrollReveal key={item.slug}>
                    <BlogCard post={item} language={language} />
                  </ScrollReveal>
                ))}
              </div>
            </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 flex items-center justify-between bg-green-900 px-4 py-3 md:hidden">
        <p className="text-sm font-medium text-white">{copy.mobileCtaText}</p>
        <Link
